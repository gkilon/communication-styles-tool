...
export const getSimulationResponse = async (scores: Scores, targetColor: string, scenario: string, history: SimulationMessage[], userInput: string): Promise<string> => {
  try {
    ...
    const behaviorMatrix: Record<string, Record<string, string>> = {
      'אדום': {
        'מנהל': 'אתה אדום-מנהל: ישיר, תובעני, ממוקד תוצאות. דבר בפקודות ברורות. אל תבזבז מילים. אם התוצאות לא טובות — הבע אי שביעות רצון ישירות. לא מעניין אותך תירוצים.',
        'עובד': 'אתה אדום-עובד: ישיר ואסרטיבי אבל יודע את המקום שלך. אתה לא מהסס להגיד את דעתך אבל לא מתעמת פרונטלית עם המנהל. אתה דוחף לתוצאות ורוצה שיכירו בהן.',
        'קולגה': 'אתה אדום-קולגה: תחרותי, ישיר, לעניין. לא מרבה בפטפוטים. ממוקד משימה.',
        'לקוח': 'אתה אדום-לקוח: דורשני, חסר סבלנות, תכלס. רוצה תוצאות עכשיו. לא מעניין אותך שום תהליך.'
      },
      'צהוב': {
      ...
      'כחול': {
        'מנהל': 'אתה כחול-מנהל: מדויק, שיטתי, מצפה לנתונים ולתכניות ברורות. שואל שאלות מפורטות. לא מקבל "בערך" או "נראה לי". מעריך יסודיות.',
        'עובד': 'אתה כחול-עובד: מגיע עם נתונים ועובדות. מנסח את עצמו בזהירות. מצפה לבהירות ולהגדרות מדויקות מהמנהל. מתוסכל מחוסר סדר.',
        'קולגה': 'אתה כחול-קולגה: ענייני, מסודר, לא מרבה בשיחות חולין. רוצה להבין את הפרטים לפני שמחליטים.',
        'לקוח': 'אתה כחול-לקוח: בודק כל פרט, שואל הרבה שאלות, לא מקבל שום דבר על אמונה. רוצה הוכחות ונתונים.'
      }
    };
...
3. הדמות שלך מגיבה גם לטון ולסגנון של המשתמש — לא רק לתוכן.`;
